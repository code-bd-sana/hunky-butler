"use client"
import { useGetButlerOverviwQuery } from "@/features/booking";
...
const ButlerRow = ({ butler }) => {
  const { data: butlerOverview, isLoading: overviewLoading } = useGetButlerOverviwQuery(butler._id);
  
  const serviceProvided = butlerOverview?.totalServiceProvided || 0;
  const totalSpent = butlerOverview?.totalSpent || 0;
  ...
  return (
    <tr className="h-[56px] bg-white hover:bg-zinc-50/60 border-b border-[#EFE7EA]">
      ...
      <td className="px-2 align-middle text-zinc-600 whitespace-nowrap">
        {butler._id?.slice(0, 6)}...
      </td>
      
      <td className="px-2 align-middle whitespace-nowrap">
        {butler?.createdAt && new Date(butler.createdAt).toLocaleDateString('en-GB', {
          day: 'numeric',
          month: 'short',
          year: 'numeric'
        })}
      ...
      <td className="px-2 align-middle whitespace-nowrap">
        {serviceProvided} Times
      </td>
      
      <td className="px-2 align-middle">{butler?.email}</td>
...
export default function ButlerList() {
  ...
  const { data, isLoading, error } = useGetAllButlerQuery({
    page: currentPage,
    limit: 10,
    ...
    return (
      <section className="w-full rounded-2xl border border-zinc-100 bg-white shadow-sm p-8">
        <div className="flex items-center justify-center">
          <p className="text-lg">Loading butlers...</p>
        </div>
      </section>
    );
  }
  ...
  if (error) {
    return (
      <section className="w-full rounded-2xl border border-zinc-100 bg-white shadow-sm p-8">
        <div className="flex items-center justify-center">
          <p className="text-lg text-red-500">Error loading butlers</p>
        </div>
      </section>
    );
  }

  const butlers = data?.data || [];
  const totalPages = data?.totalPages || 1;
  const totalButlers = data?.total || 0;
        ...
        <div className="overflow-x-auto max-w-[76vw] max-h-[500px] scrollbar-hide overflow-y-auto rounded-xl">
          <table className="w-full overflow-scroll border-collapse min-w-[820px]">
            ...
            <tbody className="text-[16px] text-[#333333]">
              {butlers.length > 0 ? (
                butlers.map((butler) => (
                  <ButlerRow key={butler._id} butler={butler} />
                ))
              ) : (
                <tr>
